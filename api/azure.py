...
async def generate_text(user_prompt: str, system_prompt: str, rag_context: str | None = None) -> str:
  if not system_prompt:
    raise HTTPException(status_code=500, detail="Missing system prompt.")
    ...
    messages.append(
      {
        "role": "system",
        "content": (
          "Use the following Bank Islami knowledge as the primary source of truth. "
          "If the answer is not present, do not guess. Ask a short clarification "
          "question or offer a human handoff.\n\n"
          f"{rag_context}"
        ),
      }
    )
